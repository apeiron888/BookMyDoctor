...
const Navbar = () => {
    ...
    return (
        <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
            <img onClick={() => navigate('/')} className='w-44 cursor-pointer' src={assets.logo} alt="BookMyDoctor Logo" />
            <ul className='hidden md:flex items-start gap-5 font-medium'>
                ...
                <NavLink to='/doctors'>
                    <li className='py-1'>{t('ALL DOCTORS')}</li>
                    <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden' />
                </NavLink>
                <NavLink to='/about'>
            ...
            <div className='flex items-center gap-4'>
                <select
                    ...
                    value={i18n.language}
                    className='bg-transparent border border-gray-400 text-sm px-2 py-1 rounded cursor-pointer'
                >
                        ...
                        ? <div onClick={() => setShowDropdown(prev => !prev)} className='flex items-center gap-2 cursor-pointer group relative'>
                            ...
                            <div className={`absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 ${showDropdown ? 'block' : 'hidden'} group-hover:block`}>
                                <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4'>
                                    ...
                                    <p onClick={() => navigate('/my-appointments')} className='hover:text-black cursor-pointer'>{t('My Appointments')}</p>
                                    <p onClick={() => logout()} className='hover:text-black cursor-pointer'>{t('Logout')}</p>

                                </div>
                            </div>
                        </div>
                        : <button onClick={() => navigate('/login')} className='bg-primary text-white px-8 py-3 rounded-full font-light hidden md:block cursor-pointer'>{t('Create account')}</button>
                ...
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all `}>
                    ...
                    <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                        ...
                        <NavLink onClick={() => setShowMenu(false)} to='/doctors'><p className='px-4 py-2 rounded inline-block'>{t('ALL DOCTORS')}</p></NavLink>
